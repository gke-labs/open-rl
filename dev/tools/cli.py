#!/usr/bin/env python3
# This file contains a command-line interface tool for interacting with the Open-RL server.
...
def get_server_url():
  return os.environ.get("BASE_URL", "http://localhost:9003")
...
def chat(args):
  ...
  while True:
    try:
      ...
      while True:
        # The server uses a 60-second timeout for long polling; set client timeout to 65s to avoid premature ReadTimeout.
        poll_resp = requests.post(f"{base_url}/api/v1/retrieve_future", json={"request_id": req_id}, timeout=65)
