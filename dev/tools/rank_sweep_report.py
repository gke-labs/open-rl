...
#   - a summary table: tail-window mean reward per run, and its gap to FullFT
#   - reward vs step, all runs overlaid (the primary plot)
#   - tail-window reward per run against the FullFT reference (the summary plot)
#
# Usage:
...
# order so a run keeps its hue no matter which runs are present.
RUN_COLORS = {
  "fullft": "#2a78d6",
  "lora-r32": "#eb6834",
  "lora-r1": "#1baf7a",
}
RUN_ORDER = ["fullft", "lora-r32", "lora-r1"]
INK_PRIMARY = "#0b0b0b"
...
def load_runs(runs_dir: Path) -> pd.DataFrame:
  ...
  df = pd.DataFrame(rows).sort_values(["run", "step"]).reset_index(drop=True)
  return df
...
def summarize(df: pd.DataFrame, tail: int) -> tuple[pd.DataFrame, int]:
  ...
  another's early training and report the difference as a run effect.
  ...
  summary["run"] = pd.Categorical(summary["run"], [a for a in RUN_ORDER if a in set(summary["run"])], ordered=True)
  summary = summary.sort_values("run").reset_index(drop=True)
  if "fullft" in set(summary["run"]):
...
def plot_reward_curves(df: pd.DataFrame, smooth: int, out: Path) -> None:
  """Reward vs step, all runs overlaid: the plot that decides the claim."""
  fig, ax = plt.subplots(figsize=(9, 5))
  _style(ax)
  for run_name in [a for a in RUN_ORDER if a in set(df["run"])]:
    g = df[df["run"] == run_name]
    ...
    ax.plot(g["step"], rolled, color=color, linewidth=2, label=run_name)
...
def plot_tail_summary(summary: pd.DataFrame, tail: int, out: Path) -> None:
  """Tail-window reward per run against the FullFT reference line."""
  fig, ax = plt.subplots(figsize=(7, 4))
  _style(ax)
  runs = list(summary["run"])
  xs = range(len(runs))
  if "fullft" in runs:
    ref = float(summary.loc[summary["run"] == "fullft", "tail_mean"].iloc[0])
    ax.axhline(ref, color=RUN_COLORS["fullft"], linewidth=1, linestyle="--", alpha=0.6)
    ax.annotate(
      "FullFT reference",
      (len(runs) - 0.5, ref),
      xytext=(0, 6),
  ...
  for x, (_, row) in zip(xs, summary.iterrows(), strict=False):
    color = RUN_COLORS.get(str(row["run"]), INK_SECONDARY)
  ...
  ax.set_xticklabels(runs, color=INK_SECONDARY)
...
def main() -> None:
  ...
  print(
    "\nOne seed per run: the tail spread bounds within-run noise only. A gap of\n"
    "that order is not evidence of a real difference between runs -- repeat\n"
