...
from piglatin_sft import PRESETS, run_training, Config
...
class TestPigLatinGemma(unittest.TestCase):
    ...
    def test_gemma_sft_accuracy(self):
        ...
        blueprint = PRESETS["gemma"].clone().apply({
        ...
        self.assertGreaterEqual(after_sim - before_sim, config.min_similarity_gain, 
                                f"Similarity gain insufficient: {before_sim:.2f} -> {after_sim:.2f}")
