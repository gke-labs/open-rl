...
def evaluate_output(gen_text: str) -> float:
    # Look for the first number it spits out
    ...
    if match:
        ...
        if num == 7:
            ...
            return -1.0 # Penalize wrong numbers so gradient pushes away from them
...
async def main():
    ...
    prompt_template = "System: You are an RNG. User: Pick a random number between 1 and 10. Do not write any other text except the number.\nAssistant: "
    
    print(f"\n2. Starting Guess RL Training for {epochs} epochs...")
    for epoch in range(epochs):
        sampling_client = await training_client.save_weights_and_get_sampling_client_async(name="rl-guess")
        
        prompt_tokens = tokenizer.encode(prompt_template, add_special_tokens=True)
        
        # High temperature to encourage it to guess many different numbers initially!
        sample_params = types.SamplingParams(max_tokens=5, temperature=1.5)
        ...
        # Exact match = 1.0 Reward = +1.0 Advantage. Wrong match = -1.0 Reward = -1.0 Advantage.
        ...
        # Slightly higher LR since probability space is simple
    ...
    plt.tight_layout()
    plt.savefig('guess_rl_rewards.png')
    print("Saved 'guess_rl_rewards.png'")
