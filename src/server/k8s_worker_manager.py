...
class KubernetesFFTWorkerManager:
  ...
  def launch(
    ...
    self.launch_trainer(model_id, base_model, strategy=strategy, diffing_device=diffing_device)
  ...
  def _launch_pod(
    ...
    try:
      pod_body = self.render_pod(pod_name, model_id, job_id, role=role, base_model=base_model, strategy=strategy, diffing_device=diffing_device)
