...
async def select_idle_pods(store: Any, pods: list[dict[str, Any]], timeout_seconds: float, now: float | None = None) -> list[dict[str, Any]]:
  """Pods belonging to a base-model group in which every member is idle past ``timeout_seconds``.

  A pod whose idle time is unknown -- busy, or lacking both a heartbeat and a
  start time -- disqualifies its whole group. Unknown means "do not reap".
  """
  ...
  for members in groups.values():
    ...
    for pod in members:
      ...
      idle = idle_seconds(heartbeat, pod.get("start_time"), now=now)
...
async def reap_idle_lora_workers(manager: Any, store: Any, timeout_seconds: float) -> list[str]:
  ...
  reaped: list[str] = []
  for pod in await select_idle_pods(store, pods, timeout_seconds):
    name = pod["name"]
    try:
...
async def run_lora_idle_reaper(manager: Any, store: Any, interval: float, timeout_seconds: float) -> None:
  while True:
    await asyncio.sleep(interval)
    try:
      reaped = await reap_idle_lora_workers(manager, store, timeout_seconds)
      if reaped:
...
def start_lora_idle_reaper(manager: Any | None, store: Any) -> asyncio.Task | None:
  ...
  if interval <= 0:
    ...
    return None
  print(f"[GATEWAY] Idle LoRA worker teardown after {timeout_seconds:.0f}s, checked every {interval:.0f}s")
  return asyncio.create_task(run_lora_idle_reaper(manager, store, interval, timeout_seconds))
