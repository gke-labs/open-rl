...
This document proposes reproducing that claim on Open-RL, running every run of
the comparison **concurrently on one cluster**. The parallel design is not
presentational: runs launched together see identical cluster state, dataset, and
...
- **Throughput under sharing.** Wall-clock per step for runs multiplexed on one
  GPU versus a run holding its own.
...
Three runs on one base model, differing only in the variable under test:

| Run | Mode | Rank | Learning rate |
|---|---|---|---|
| A | FullFT | — | 1e-5 |
| B | LoRA | 32 | 1e-4 |
| C | LoRA | 1 | 1e-4 |
...
LR across modes would compare each run at a different distance from its own
optimum and is the more misleading experiment. The 10× ratio is the paper's
finding; we adopt it.

### 3.2 Concurrency and expected placement

All three runs launch together against one gateway. Given the tiering and claim
rules already in the scheduler, the expected topology on a small base model is:

- Runs **B and C share one trainer worker and one sampler worker**. Both are
...
- Run **A takes its own trainer and sampler claims**, since FFT workers do not
  share adapters.
...
size-aware tiering: before it, any FFT run would have demanded an 80 GB device.

### 3.3 Measurement

Primary axis is **reward versus step**, not wall-clock. Runs B and C share a GPU
while A does not, so throughput differs by construction; per-step learning is
the only fair comparison. Each run writes metrics to its own log path, and the
comparison is a join across runs on step index.
...
1. **Reward vs. step, all runs overlaid** — the primary result. Confirmation
   looks like three curves inside one noise band; refutation looks like run C
...
4. **Wall-clock per step, per run** — the systems half. Shows the cost of
...
4. **Single seed.** One seed per configuration cannot separate a real gap from noise.

---

## 6. Phasing

**Phase 1 — one seed, small model.** Three runs, short training, confirm the
placement topology and that all three train. Produces plots 1, 4 and 5.
Primarily a validation that the harness measures what we think.

**Phase 2 — repeat seeds.** Three runs × N seeds for the noise band, enabling
...
- Producing benchmark-quality absolute scores. The claim is about runs relative
