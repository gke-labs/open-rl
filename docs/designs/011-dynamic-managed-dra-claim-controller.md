...
- **Startup is busy.** A worker publishes `busy` *before* it loads its model, not after. Loading takes minutes during which the worker has no requests to point at, and the reaper cannot otherwise tell "no work has arrived yet" from "no work is happening". Skipping this reaped a booting sampler together with the trainer that was waiting on it, mid-job — the group check cannot help when every member of the group looks equally idle. For the same reason `last_activity` is stamped on the busy→idle transition as well as the busy one: a worker busy up to this instant was *active* up to this instant, and dating the idle clock from the start of the work would make anything slower than the timeout finish already expired.
- A worker that has published nothing is unknown, not idle, and silence has two causes wanting opposite handling: still coming up (must not be reaped) versus an older image or a process that died before its first write (must not be immortal). `OPEN_RL_LORA_WORKER_STARTUP_GRACE_SECONDS` (default `900`) separates them — below it the pod is left alone, past it it ages out from its own start time. The window must cover the image pull too, since the pod's start time predates it. A heartbeat older than the pod itself is discarded as belonging to the previous pod of the same (deterministic) name.
