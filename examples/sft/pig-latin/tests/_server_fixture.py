...
SERVER_DIR = CLIENT_DIR.parent.parent.parent / "src" / "server"
...
class OpenRlServerCase(unittest.TestCase):
  ...
  def setUpClass(cls) -> None:
    if cls.REQUIRE_HF_TOKEN and not os.getenv("HF_TOKEN"):
      print("\n" + "=" * 70)
      print(f"[SKIP] The test suite for {cls.BASE_MODEL} was skipped.")
      print("       Reason: HF_TOKEN environment variable is missing (required for gated models).")
      print("=" * 70 + "\n")
    ...
    cls._server = subprocess.Popen(
      ["uv", "run", "--extra", "cpu", "python", "-m", "uvicorn", "gateway:app", "--host", "127.0.0.1", "--port", str(port)],
      cwd=str(SERVER_DIR),
      env=env,
