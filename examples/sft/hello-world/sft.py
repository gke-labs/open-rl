"""
Tinker SFT Sandbox Demonstration Script

This file contains a generic script for Supervised Fine-Tuning (SFT) demonstration
using the Tinker SDK with the Open-RL server.

Purpose:
    It trains a model to output a specific, constant target answer (default "foo")
    for a set of hardcoded questions, serving as a "hello world" for the training infrastructure.

Modes of Operation:
    - Single Tenant: Runs a single training job.
    - Parallel (Multi-Tenant): Runs two training jobs concurrently to demonstrate the server's multi-tenant capabilities
      (handling multiple adapters without gradient poisoning).
...
Arguments:
    ...
    --epochs: Number of training steps (default: 10).
    --target: The target text the model should learn to output (default: "foo").
    --parallel: Flag to run in parallel mode.

Output:
    Saves a loss curve plot as 'sft_loss.png' or 'sft_loss_parallel.png'.
"""
...
os.environ["TINKER_BASE_URL"] = os.environ.get("BASE_URL", "http://localhost:9003")
...
async def main():
  ...
  def update_plot(client_name, history):
    ...
    filename = artifacts_dir / ("sft_loss_parallel.png" if args.parallel else "sft_loss.png")
