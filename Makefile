...
run-server-engine-sampler:
	@-kill -9 $$(lsof -ti:8000) 2>/dev/null || true
	cd server && ENABLE_GCP_TRACE=$(ENABLE_GCP_TRACE) UV_INDEX_URL="https://pypi.org/simple" SAMPLER_BACKEND=engine VLLM_MODEL="$(VLLM_MODEL)" uv run --extra cpu uvicorn src.gateway:app --host 127.0.0.1 --port 8000

run-function-gemma-server:
	@-kill -9 $$(lsof -ti:9000) 2>/dev/null || true
	cd server && OPEN_RL_SINGLE_PROCESS=1 SAMPLER_BACKEND=engine OPEN_RL_BASE_MODEL="google/functiongemma-270m-it" PYTHONUNBUFFERED=1 uv run --extra cpu uvicorn src.gateway:app --host 127.0.0.1 --port 9000 $(ARGS)
...
run-pig-latin-server:
	...
	cd server && ENABLE_GCP_TRACE=$(ENABLE_GCP_TRACE) UV_INDEX_URL="https://pypi.org/simple" OPEN_RL_SINGLE_PROCESS=1 OPEN_RL_BASE_MODEL="Qwen/Qwen3-0.6B" SAMPLER_BACKEND=engine VLLM_MODEL="Qwen/Qwen3-0.6B" uv run --extra cpu uvicorn src.gateway:app --host 127.0.0.1 --port 9001
...
run-pig-latin-gemma-server:
	...
	cd server && ENABLE_GCP_TRACE=$(ENABLE_GCP_TRACE) UV_INDEX_URL="https://pypi.org/simple" OPEN_RL_SINGLE_PROCESS=1 OPEN_RL_BASE_MODEL="google/gemma-3-1b-it" SAMPLER_BACKEND=engine VLLM_MODEL="google/gemma-3-1b-it" uv run --extra cpu uvicorn src.gateway:app --host 127.0.0.1 --port 9002
