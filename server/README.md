...
- local single-process training (CPU PyTorch):
  `cd server && uv sync --extra cpu && OPEN_RL_SINGLE_PROCESS=1 SAMPLER_BACKEND=engine OPEN_RL_BASE_MODEL="Qwen/Qwen3-0.6B" uv run --extra cpu uvicorn src.gateway:app --host 127.0.0.1 --port 9001`
